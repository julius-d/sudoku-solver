...
export default class OnlyOnePlaceVerticalLine extends AbstractFinder {
  ...
  finderLogic(cantBes: Array<SudokuEvent>) : Array<SudokuEvent> {
    ...
    cantBes.forEach(cantBe => {
      // @ts-ignore
      const canBeForNumberInLine = this.speicher.get(cantBe.getPosition().getYKoordinate()).get(cantBe.getNumber()) || []; // TOOD handle undefine
      if (!canBeForNumberInLine.find(it => it.getXKoordinate() === cantBe.getPosition().getXKoordinate()
        ...
        if (canBeForNumberInLine.length === 8) {
          result.push(new SudokuEvent(SudokuEventType.NUMBER_FOUND, OnlyOnePlaceVerticalLine.onlyPossiblePosition(canBeForNumberInLine), cantBe.getNumber(), this.name  ))
        }
      }
