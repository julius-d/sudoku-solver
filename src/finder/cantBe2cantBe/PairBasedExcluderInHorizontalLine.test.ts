...
describe("PairBasedExcluderInHorizontalLine", () => {
  ...
  it("finds exclusions", () => {
    let pairBasedExcluderInHorizontalLine =
      new PairBasedExcluderInHorizontalLine();
    let cantBeFoundEvent: CantBeFoundEvent[] =
      pairBasedExcluderInHorizontalLine.finderLogic([
  ...
  it("handles multiple numbers", () => {
    ...
    const sudokuPositions3 = cantBeFoundEvent
      .filter((it) => it.getNumber() === 3)
      .map((it) => it.getPosition());
    const sudokuPositions4 = cantBeFoundEvent
